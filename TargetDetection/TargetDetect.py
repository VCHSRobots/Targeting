...
def CommOutbound(recipiant, message):
	if recipiant == "RPiConsole":
		...
		logmsg = strftime(%H:%M:%S, time.time()) + " [INFO] Communication stream: RPi --> RPiConsole: " + message
		with open('log.txt', 'a') as f:
			print(logmsg, file=f)
	elif recipiant == "RoboRio":
		#Code HERE to send "message" to RoboRio
		#Log the communication stream:
		logmsg = strftime(%H:%M:%S, time.time()) + " [INFO] Communication stream: RPi --> RoboRio: " + message
		with open('log.txt', 'a') as f:
			print(logmsg, file=f)
	elif recipiant == "Op":
		#Log the communication stream:
		logmsg = strftime(%H:%M:%S, time.time()) + " [INFO] Communication stream: RPi --> Op: " + message
		with open('log.txt', 'a') as f:
			print(logmsg, file=f)
	...
	if sender == "RoboRio":
		if mode == "Time":
			...
			while messagesrecieved < value:
				...
				#Log the communication stream:
				logmsg = strftime(%H:%M:%S, time.time()) + " [INFO] Communication stream: RoboRio --> RPi: " + message
				with open('log.txt', 'a') as f:
					print(logmsg, file=f)
				messagesrecieved += 1
	elif sender == "Op":
		if mode == "Time":
			...
			while messagesrecieved < value:
				#Code for listening goes HERE. Only continue with code past this point when a message is properly recieved.
				#Log the communication stream:
				logmsg = strftime(%H:%M:%S, time.time()) + " [INFO] Communication stream: Pi --> Op: " + message
				with open('log.txt', 'a') as f:
					print(logmsg, file=f)
				messagesrecieved += 1
	elif sender == "All":
